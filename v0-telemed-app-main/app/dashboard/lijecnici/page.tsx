...
export default async function LijecniciPage({
  ...
  let query = supabase.from("lijecnici").select("*").order("prezime", { ascending: true })
  ...
  const { data: specijalizacijeData } = await supabase.from("lijecnici").select("specijalizacija")
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Upravljanje lijecnicima u sustavu</p>
        </div>
