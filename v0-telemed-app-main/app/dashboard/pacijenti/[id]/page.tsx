...
export default async function PacijentDetaljPage({ params }: { params: { id: string } }) {
  ...
  const { data: nalazi } = await supabase
    ...
    .select(
      ...
      dijagnoza,
      terapija,
      lijecnici(ime, prezime, specijalizacija)
    `,
    )
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Dr. {nalaz.lijecnici?.ime} {nalaz.lijecnici?.prezime} - {nalaz.lijecnici?.specijalizacija}
                      </p>
                ...
                <div key={termin.id} className="border-b pb-4 last:border-b-0 last:pb-0">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Dr. {termin.lijecnici?.ime} {termin.lijecnici?.prezime} - {termin.lijecnici?.specijalizacija}
                      </p>
                    </div>
