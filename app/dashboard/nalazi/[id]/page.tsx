  ...
  const { data: nalaz, error } = await supabase
    ...
    .select(
      ...
      *,
      ...
      liječnici(id, ime, prezime, specijalizacija)
    `,
    )
    .eq("id", params.id)
    .single()
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div>
              <p className="text-sm text-muted-foreground">Ime i Prezime</p>
              <p className="font-medium">
                <Link href={`/dashboard/lijecnici/${nalaz.liječnici.id}`} className="hover:underline">
            ...
            <div>
              ...
              <Badge variant="secondary">{nalaz.liječnici.specijalizacija}</Badge>
            </div>
          </CardContent>
        </Card>
      </div>
